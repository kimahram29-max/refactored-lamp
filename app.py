...
st.markdown("""
    <style>
        ...
        .login-box { max-width: 500px; margin: 100px auto; padding: 40px; background-color: white; border-radius: 16px; box-shadow: 0 10px 15px -3px rgba(0,0,0,0.1); text-align: center; }
    </style>
...
if "user_role_assigned" not in st.session_state:
    st.session_state.user_role_assigned = None
...
# --- [1단계] 구글 인증 오브젝트 생성 ---
# Streamlit Cloud의 Secrets 서비스에 저장된 가치들을 불러옵니다.
authenticator = Authenticate(
    secret_credential_path=st.secrets["textkey"],
    cookie_name="lms_user_cookie",
...
# --- [2단계] 로그인 여부에 따른 화면 분기 ---
if not st.session_state.get("connected", False):
    # 로그인 전 화면
    st.markdown("""
        <div class='login-box'>
            <h2 style='color: #1E3A8A;'>🎓 AI 영어 글쓰기 LMS</h2>
            <p style='color: #64748B;'>본 시스템은 구글 계정 로그인이 필요합니다.<br>아래 버튼을 눌러 인증을 진행해 주세요.</p>
        </div>
    """, unsafe_allow_html=True)
    ...
    user_name = user_info.get("name", "학습자")

    # --- [3단계] 역할 설정 화면 (최초 1회만 노출) ---
    if st.session_state.user_role_assigned is None:
        st.markdown("<div class='login-box'>", unsafe_allow_html=True)
        st.markdown(f"### 🎉 환영합니다, {user_name}님!")
        st.markdown("<p style='color: gray;'>시스템 이용을 위해 역할을 한 번만 선택해 주세요.</p>", unsafe_allow_html=True)
        
        selected_role = st.selectbox("당신의 역할은 무엇입니까?", ["선택하세요", "🌱 학생 (Learner)", "💼 교사 (Instructor)"])
        
        if selected_role != "선택하세요":
            if st.button("LMS 시작하기", use_container_width=True):
                st.session_state.user_role_assigned = selected_role
                st.rerun()
        st.markdown("</div>", unsafe_allow_html=True)

    # --- [4단계] 역할에 따른 메인 LMS 화면 진입 ---
    else:
        # 사이드바 프로필 영역 및 로그아웃
        with st.sidebar:
            st.markdown(f"<h3 style='text-align: center; color:#F1F5F9;'>👤 {user_name}님</h3>", unsafe_allow_html=True)
            st.markdown(f"<p style='text-align: center; color:#94A3B8; font-size:12px;'>{user_email}</p>", unsafe_allow_html=True)
            st.info(f"현재 모드: {st.session_state.user_role_assigned}")
            st.write("---")
            # 로그아웃 버튼
            authenticator.logout()
            st.write("---")
            st.markdown("<small style='color:#94A3B8;'>EduTech LMS v1.5</small>", unsafe_allow_html=True)
        ...
        # ==================== [학생 화면] ====================
        if "학생" in st.session_state.user_role_assigned:
